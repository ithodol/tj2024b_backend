...
public class MemberDto { // DTO : 서로 다른 레이어(MVC패턴)들 간의 데이터를 이동하는 객체
	...
	// * 주로 DTO는 멤버변수 private 접근제한자를 사용한다. / private 이란? 다른 클래스에서 접근 불가능
		// - getter / setter 메소드 : private 멤버변수를 다른 클래스에 간접 접근하기 위한 메소드
		// - toString()메소드 : 객체 호출시 참조(주소)값 대신 멤버변수 값을 반환하는 메소드 *객체 내용 확인용
	// * 주로 DTO는 생성자를 기본적으로 2개(디폴트, 풀생성자), 그 외 커스텀 / 생성자란? new(인스턴스/객체)를 생성할 때 사용되는 초기화 함수 역할
		// - 오버로딩 : 동일한 메소드명을 여러개 선언 가능,	전제조건 : 매개변수의 타입, 갯수, 순서가 달라야함 
