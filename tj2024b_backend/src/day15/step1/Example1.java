...
public class Example1 {
	public static void main(String[] args) {
		// 1. 동물 객체(인스턴스) 생성
		동물 animal = new 동물(); animal.showMe(); // [동물 생성자 탄생], myName : 동물
		// 2. 조류 객체(인스턴스) 생성
			// **** 하위 클래스의 인스턴스를 생성하면 상의 클래스의 인스턴스도 같이 생성된다 ****
		조류 bird = new 조류();	bird.showMe(); // [동물 생성자 탄생], [조류 생성자 탄생], myName : 조류
		// 3. 참새 객체(인스턴스) 생성
			// **** 하위 클래스의 생성자를 호출하면 상위 클래스의 생성자도 같이 호출된다 ****
		참새 sparrow = new 참새(); sparrow.showMe(); // [동물 생성자 탄생], [조류 생성자 탄생], [참새 생성자 탄생], myName : 참새

		// [타입변환]
			// 참새는 동물과 조류가 될 수 있다.
			// 즉, 자식 타입의 자료는 부모 타입으로 자동 변환 가능 / 자식 객체는 부모타입을 참조한다. 
		조류 bird2 = sparrow; // 참새 타입의 자료(객체)가 조류 타입으로 변환된다. (자동타입변환)
		동물 animal2 = sparrow; // 참새 타입의 자료(객체)가 동물 타입으로 변환된다. (자동타입변환)
		Object obj2 = sparrow; // 참새 타입의 자료(객체)가 Object 타입으로 변환된다. (자동타입변환)
			// 동물은 동물과 조류가 될 수 없다.
